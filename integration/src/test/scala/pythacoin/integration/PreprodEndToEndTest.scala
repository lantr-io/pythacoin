...
import pythacoin.bot.{Bot, BotConfig, BotCtx, Wallet}
...
/** End-to-end smoke test against preprod or preview, exercising the full bot stack with a real
  * ChainStore-backed warm restart, real Pyth Lazer WS, and a deterministic test CDP that's
  * idempotent across runs.
  *
  * See `liquid-bot/doc/end-to-end-test.md` for the design. The hooks this test relies on (Bot
  * instance fields for live introspection, Mithril bootstrap on empty store) all live in main; this
  * file is the assertion harness.
  *
  ...
  *   - Standard `BotConfig.fromEnv` vars: `BLOCKFROST_API_KEY`, `PYTH_POLICY_ID`, `PYTH_KEY`,
  *     `PYTHACOIN_BOT_ADDR`, `PYTHACOIN_BOT_KEY`, `PYTHACOIN_BOT_VKEY`.
  *   - `PYTHACOIN_CHAIN_STORE_DIR` — persistent RocksDB path.
  *
  * Optional:
  *   - `PYTHACOIN_E2E_DURATION_SECONDS` — run window (default 180).
  *   - `PYTHACOIN_E2E_CLEANUP` — `true` closes the test CDP on success (default leaves it on chain
  *     so the next run is faster).
  *   - `PYTHACOIN_BOOTSTRAP=mithril` (+ `PYTHACOIN_MITHRIL_*`) — fast first run via a Mithril
  *     snapshot rather than full sync from genesis.
  */
class PreprodEndToEndTest extends AnyFunSuite {

    private val log = LoggerFactory.getLogger(classOf[PreprodEndToEndTest])

    /** AssetName max is 32 bytes. 4-byte prefix + 28-byte AddrKeyHash fits exactly, keeping the
      * full PKH for per-wallet uniqueness.
      */
    private val NftPrefix: Array[Byte] = "e2e:".getBytes("UTF-8")

    /** Bot defaults `priceMaxAgeSeconds` to 60; 50 ADA / 5 PUSD opens at any plausible ADA/USD
      * price (LTV ~10% even at $0.05/ADA), so the health margin is wide enough that natural price
      * movement during the test window can't make this CDP a liquidation candidate.
      */
    ...
    /** Spin up the bot in a supervised scope, poll the three forward-progress assertions directly
      * against the live `Bot` fields, and exit the scope as soon as they all pass (or the deadline
      * fires).
      *
      * Falling out of `supervised` cancels the bot fork; the fork's chain follower is
      * interrupt-aware, so this is the clean shutdown path.
      */
    private def runBotAndAssert(cfg: BotConfig, nftHex: String, durationSeconds: Long): Unit = {
        supervised {
            ...
            log.info("Bot started; polling assertions")

            waitForCond(deadlineMs, s"chain snapshot contains test CDP $nftHex") {
                bot.cdps
                    .snapshot()
                    .exists(view => view.values.exists { case (_, info) => info.nftName == nftHex })
            }
            ...
            waitForCond(deadlineMs, "evaluator ran ≥ 1 decision pass") {
                bot.evaluator.evaluationsRun > 0
            }
            log.info(
              "All assertions passed — " +
                  s"evaluationsRun=${bot.evaluator.evaluationsRun}, " +
                  s"liquidationCandidates=${bot.evaluator.liquidationCandidates}"
            )
        }
    }
    ...
    /** Reuse if present (idempotent across runs — the warm chain store and the deterministic NFT
      * name make this the fast path on the second run); otherwise open a fresh small healthy CDP
      * via the off-chain tx-builder + Blockfrost submission, and wait for confirmation.
      */
    ...
    /** Best-effort cleanup. Failures log-and-continue rather than failing the test — the primary
    ...
    /** Stable per-wallet NFT name: 4-byte ASCII prefix + 28-byte payment-key-hash. Two operators
      * running concurrently can't collide because the PKH is unique, and the same wallet finds the
      * same CDP on every run.
      */
