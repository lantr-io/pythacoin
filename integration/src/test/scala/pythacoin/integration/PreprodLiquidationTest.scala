...
/** Runs the bot against **real preprod + real Pyth Lazer** as a dry-run smoke
  * check. Tagged so `sbt integration/test` doesn't try to talk to preprod or
  * eat operator credits.
  *
  * What this verifies (when run, in dry-run mode):
  *   - `BotApp` connects to a preprod relay via N2N and follows the chain.
  *   - `PriceStream` opens a real Pyth Lazer WS subscription, decodes pushes,
  *     and populates `priceCache` within `priceMaxAgeSeconds`.
  *   - `Evaluator` runs at least one decision pass against live data without
  *     crashing.
  *
  * What this does NOT verify:
  *   - That a liquidation actually fires — natural ADA/USD movement during a
  *     short window is unpredictable. To force a fire, lower
  *     `PYTHACOIN_MIN_LTV_BPS` enough that an existing CDP becomes liquidatable.
  *
  * Required env vars (all standard `BotConfig.fromEnv` ones):
  *   - `BLOCKFROST_API_KEY`, `PYTH_POLICY_ID`, `PYTH_KEY`,
  *     `PYTHACOIN_BOT_ADDR`, `PYTHACOIN_BOT_KEY`, `PYTHACOIN_BOT_VKEY`.
  *
  * Override `runDurationSeconds` (env `PYTHACOIN_PREPROD_TEST_SECONDS`) to
  * extend or shorten the observation window.
  */
class PreprodLiquidationTest extends AnyFunSuite {

    private val log = LoggerFactory.getLogger(classOf[PreprodLiquidationTest])

    /** How long to keep the bot running. Default: 90 s — long enough to see
      * several WS pushes (5×/sec at fixed_rate@200ms) and at least one
      * chain-tip catch-up.
      */
