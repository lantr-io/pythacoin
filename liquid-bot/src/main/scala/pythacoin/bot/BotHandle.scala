...
/** Observability handle into a running bot — used by integration tests instead
  ...
  * created it.
  *
  * The telemetry accessors below (`chainSnapshot`, `evaluationsRun`, etc.) are
  * non-blocking atomic reads. `ctx` exposes the live [[BotCtx]] for tests
  * that need the queries / tx-builders / providers; calls into those are
  * real I/O and may block.
  */
